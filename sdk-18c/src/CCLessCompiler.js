 ...
 */
exports.compileAuto = function (siteName) {
...
function doCompile (siteName) {
  endPointTransceiver.getSites().then((result) => {
    const sites = result.data.items

    const site = sites.find((el) => {
      return el.repositoryId === siteName || el.name === siteName
    })
...
function getAllComponentLess () {
  ...
  glob("widget/**/*.less").forEach((path) => {
    ...
    if (instanceMD && instanceMD.descriptorRepositoryId && instanceMD.repositoryId) {
